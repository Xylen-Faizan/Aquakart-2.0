...
export default function HomeScreen() {
  ...
  const loadHomeData = async () => {
    try {
      ...
      const { data: subData } = await supabase.rpc(
        "get_customer_subscription_status",
      );
      if (subData) {
          ...
          >
            {[
              ...
              <Pressable
                ...
                onPress={() =>
                  router.push(`/(customer)/brand/${brand.name}` as any)
                }
              >
                <View
                  style={[
            ...
            {suppliers.length > 0 ? (
              suppliers.slice(0, 3).map((supplier) => (
                <Pressable
                  ...
                  onPress={() =>
                    router.push(`/(customer)/supplier/${supplier.id}`)
                  }
                >
                  ...
                  <View style={styles.supplierStatusRow}>
                    <View
                      style={[
                        styles.statusDot,
                        {
                          backgroundColor: supplier.is_accepting_orders

...
export default function SearchScreen() {
  ...
  const [results, setResults] = useState<{ id: string; name: string; type: 'supplier' | 'product'; image?: string }[]>([]);
  ...
  const performSearch = async (searchTerm: string) => {
    ...
    try {
      ...
      const combined: any[] = [];
      if (suppliers) {
        combined.push(...suppliers.map(s => ({ id: s.id, name: s.business_name, type: 'supplier' })));
      }
      if (products) {
        combined.push(...products.map(p => ({ id: p.id, name: p.name, type: 'product', image: p.image_url })));
      }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      ...
      {loading && query.length > 1 ? (
        <View style={{ marginTop: 40 }}><LoadingState message="Searching..." /></View>
      ) : (
        <FlatList
          data={results}
          ...
          renderItem={({ item }) => (
            <Pressable style={styles.resultItem} onPress={() => handleSelect(item)}>
              ...
              <View style={styles.resultInfo}>
                ...
                <Text style={styles.resultType}>{item.type === 'supplier' ? 'Supplier' : 'Product'}</Text>
              </View>
          ...
          ListEmptyComponent={
            query.length > 1 ? (
              ...
              <View style={styles.emptyState}>
                <Ionicons name="water-outline" size={48} color={theme.colors.primaryLight} />
                <Text style={styles.emptyText}>Search for your favorite brands and local suppliers.</Text>
              </View>
            )
          }
        />

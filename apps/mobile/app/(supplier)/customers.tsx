...
export default function CustomersScreen() {
  ...
  const [selectedCustomer, setSelectedCustomer] =
    useState<SupplierCustomer | null>(null);
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const data = await CustomerService.getCustomers({
        search: searchQuery,
        customer_type:
          activeFilter !== "All" && activeFilter !== "Credit Due"
            ? (activeFilter as CustomerType)
            : undefined,
      });
      setCustomers(data);
    } catch (error) {
  ...
  const totalJarsOut = customers.reduce(
    (sum, c) => sum + (c.jar_balance || 0),
    0,
  );
            ...
            displayedCustomers.map((customer) => (
              <Card key={customer.id} style={styles.customerCard}>
                ...
                <View style={styles.tagsRow}>
                  <Badge
                    variant={
                      customer.customer_type === "household"
                        ? "neutral"
                        : "warning"
                    }
                ...
                <View style={styles.cardActions}>
                  ...
                  <Button
                    ...
                    size="sm"
                    style={styles.actionBtn}
                    onPress={() => handleViewLedger(customer)}
                  />
                </View>
              </Card>
            ))
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.typeSelector}>
              <TouchableOpacity
                style={[
                  styles.typeBtn,
                  newType === "household" && styles.typeBtnActive,
                ]}
              ...
              <TouchableOpacity
                style={[
                  styles.typeBtn,
                  newType === "office" && styles.typeBtnActive,
                ]}

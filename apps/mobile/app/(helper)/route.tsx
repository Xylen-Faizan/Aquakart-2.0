...
export default function HelperRoute() {
  ...
  const handleCompleteStop = async (stopId: string, customerName: string) => {
    Alert.alert("Mark Delivered", `Confirm delivery to ${customerName}?`, [
  ...
  const getStopStatusStyle = (status: string) => {
    switch (status) {
      case "delivered":
        return { bg: "#22C55E20", color: "#22C55E", label: "✓ Delivered" };
      case "skipped":
        return { bg: "#EF444420", color: "#EF4444", label: "✕ Skipped" };
      case "en_route":
        return { bg: "#F59E0B20", color: "#F59E0B", label: "→ En Route" };
      default:
        return { bg: "#64748B20", color: "#64748B", label: "○ Planned" };
    }
  };
        ...
        {stops.length === 0 ? (
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptySubtitle}>
              No delivery stops for today's run.
            </Text>
          </View>
        ) : (
          stops.map((stop, index) => {
            ...
            const isActive =
              stop.status === "planned" || stop.status === "en_route";
            const isOpportunistic = stop.stop_type === "opportunistic";

            return (
              <View
                key={stop.id}
                style={[styles.stopCard, isActive && styles.activeStopCard]}
              >
                {/* Sequence + Type badge */}
                ...
                <View style={styles.productRow}>
                  ...
                  <Text style={styles.productText}>
                    {stop.quantity}× {stop.products?.name || "Product"}{" "}
                    {stop.products?.size || ""}
                  </Text>
                ...
                {isActive && (
                  <View style={styles.actionRow}>
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handleCompleteStop(stop.id, stop.profiles?.name)
                      }
                    >
                      <Ionicons

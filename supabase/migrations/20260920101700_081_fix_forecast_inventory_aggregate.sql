-- 081_fix_forecast_inventory_aggregate.sql
-- Fixes issue where get_supplier_forecast returns 0 rows if supplier_inventory doesn't exist yet
...
BEGIN
    ...
    inventory AS (
        -- FIX: Added SUM() to ensure this CTE always returns 1 row, even if no inventory record exists yet
        SELECT COALESCE(SUM(available), 0)::INT AS jars

-- 083_fix_manifest_joins.sql

CREATE OR REPLACE FUNCTION public.get_today_manifest()
 RETURNS TABLE(customer_id uuid, customer_name text, customer_type customer_type_enum, phone text, address text, sector text, supplier_product_id uuid, quantity integer, effective_unit_price numeric, expected_amount numeric, jar_balance_before integer, next_delivery_date date, source text, order_id uuid, status text)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE

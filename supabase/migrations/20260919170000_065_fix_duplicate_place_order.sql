-- 065_fix_duplicate_place_order.sql

-- Drop the incorrectly ordered place_order function from an older migration
DROP FUNCTION IF EXISTS public.place_order(UUID, UUID, UUID, INT, VARCHAR, VARCHAR);

-- Ensure the correct one from 061 remains properly defined

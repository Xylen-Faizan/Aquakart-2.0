-- 040_phase7_hardening.sql
-- Phase 7: Production Hardening (Concurrency & Idempotency)

BEGIN;

-- 1. Create idempotency tracking table
...
CREATE POLICY "Users can view their own idempotency keys" ON public.api_idempotency
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own idempotency keys" ON public.api_idempotency
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- 2. Update place_order with Idempotency and FOR UPDATE Row Locking
